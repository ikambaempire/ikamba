...
import { Send, Upload, X, Image, FileVideo, Sparkles, Copy, Check, Loader2 } from "lucide-react";
...
const CaptionGenerator = () => {
  ...
  const sendMessage = async () => {
    ...
    const userMsg: Message = { role: "user", content: trimmed || "Generate a caption for these files.", files: fileInfo.length > 0 ? fileInfo : undefined };
        ...
        <div className="border-b border-border bg-card">
          <div className="max-w-4xl mx-auto px-4 py-6 text-center">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-center gap-2 mb-2">
              ...
              <h1 className="text-2xl md:text-3xl font-extrabold text-foreground">Free SEO Caption Generator</h1>
            </motion.div>
          ...
          <div className="max-w-4xl mx-auto px-4 py-6 space-y-4">
            {messages.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
                ...
                <h2 className="text-xl font-bold text-foreground mb-2">What are you posting about?</h2>
                <p className="text-muted-foreground text-sm max-w-md mx-auto mb-6">
                  Tell us about your post — the platform, topic, tone — and we'll generate the perfect SEO caption. You can also upload images or videos for context.
                </p>
                <div className="flex flex-wrap justify-center gap-2">
                  {["Instagram product launch", "LinkedIn thought leadership", "YouTube video description", "TikTok trending content"].map(suggestion => (
                    <button
                      key={suggestion}
                      onClick={() => { setInput(`I'm creating a post about: ${suggestion}`); }}
            ...
            {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-start">
                <div className="bg-card border border-border rounded-xl px-4 py-3 flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">Crafting your caption...</span>
                </div>
              </motion.div>
            )}
          ...
          <div className="max-w-4xl mx-auto px-4 py-3">
            ...
            <div className="flex items-end gap-2">
              ...
              <Textarea
                ...
                placeholder="Describe your post... (platform, topic, tone, hashtags)"
            ...
            <p className="text-[10px] text-muted-foreground mt-1.5 text-center">
              Free tool by Ikamba — captions are AI-generated. Review before posting.
            </p>
          </div>
        </div>
      </div>
